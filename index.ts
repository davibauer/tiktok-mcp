  ...
  inputSchema: {
    ...
    properties: {
      tiktok_url: {
        type: "string",
        description:
          "TikTok video URL, e.g., https://www.tiktok.com/@username/video/1234567890 or https://vm.tiktok.com/1234567890",
      },
    },
    required: ["tiktok_url"],
  },
...
const GET_POST_COMMENTS: Tool = {
  name: "tiktok_get_post_comments",
  description:
    "Get the comments of a TikTok post." +
    "This is used for getting the comments of a TikTok post." +
    "Supports TikTok video url as input." +
...
function isAvailableSubtitleArgs(
  args: unknown
): args is { tiktok_url: string } {
  return (
    typeof args === "object" &&
    args !== null &&
    "tiktok_url" in args &&
...
function isGetPostCommentsArgs(args: unknown): args is { tiktok_url: string } {
  return (
    typeof args === "object" &&
    args !== null &&
    "tiktok_url" in args &&
...
async function performGetPostComments(tiktok_url: string) {
  const url = new URL("https://tikneuron.com/api/mcp/post-comments");
  url.searchParams.set("tiktok_url", tiktok_url);
  ...
  const data = (await response.json()) as PostComments;
  ...
  if (data.success && data.comments && data.comments.length > 0) {
    ...
    return "No comments available or operation failed.";
  }
}
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  try {
    ...
    switch (name) {
      ...
      case "tiktok_get_post_comments": {
        if (!isGetPostCommentsArgs(args)) {
          throw new Error("Invalid arguments for tiktok_get_post_comments");
        }
        const { tiktok_url } = args;

        const results = await performGetPostComments(tiktok_url);
        return {
          content: [{ type: "text", text: results }],
          isError: false,
        };
